...
<body>
    <div class="row">
        ...
        <div style="min-height: 100vh; margin-left: 300px;">
            ...
            <div id="NhanVien">
                <h1 class="txt-center" style="font-size: 40px; color: #000;">Thông tin Nhân viên</h1>
                <div class="item-center">
                    <div class="row">
                        <form action="NhanVien.php" method="GET" class="row" style="margin-top: 20px; margin-right: 40px;">
                        ...
                        <a href="./ThemNhanVien.php"><button class="themthuoc select"> + Thêm mới</button></a>
                    </div>
                    <div>
                        <table class="table">
                            <tr class="col0">
                                <th class="tt" style="width: 250px; background: rgb(92,152,245);">Nhân viên</th>
                                <th class="tt" style="width: 200px; background: rgb(24,205,229);">Ngày sinh</th>
                                <th class="tt" style="width: 100px; background: rgb(92,152,245);">Giới tính</th>
                                <th class="tt" style="width: 250px; background: rgb(24,205,229);">Chức vụ</th>
                                <th class="tt" style="width: 200px; background: rgb(92,152,245);">Giới tính</th>
                            </tr>
                            <?php
                            if (isset($_GET['btnSearch'])) {
                                $key = addslashes($_GET['txtSearch']);
                                $conn = new mysqli('localhost', 'root', '', 'quanlykhothuoc');
                                $sql = "SELECT * FROM `nhan_vien` JOIN chuc_vu ON nhan_vien.Ma_Chuc = chuc_vu.Ma_Chuc WHERE Ten_NV LIKE '%$key%' ORDER BY Ten_NV";
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
										...
										<tr class="col1">
											...
											<th class="tt txt-left">' . $row['Ngay_Sinh'] . '</th>
                                ...
                                $conn = new mysqli('localhost', 'root', '', 'quanlykhothuoc');
                                $sql = "SELECT * FROM `nhan_vien` JOIN chuc_vu ON nhan_vien.Ma_Chuc = chuc_vu.Ma_Chuc ORDER BY Ten_NV";
                                $result = $conn->query($sql);
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        echo '
